...
$$
\text{Var}(M_i)= \text E[M_i^2] - \mu_{M_i}^2 = \sum^{n}_{k=1} \omega_kE[X_k^2] - \mu_{M_i}^2 =
...
calcAssim <- function(mist,gamma=F){
  if(gamma)
    Assim <- with(mist, (sum(probs*momentoGama(3,lam,a))-3*media*var-media^3)/var^1.5)
  return(Assim)
}

calcAssim(m$mist4,gamma=T)
```
